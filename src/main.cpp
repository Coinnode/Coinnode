...
unsigned int nStakeMinAge = 4 * 60 * 60; // 24 hours
...
const string strMessageMagic = "Coinnode Signed Message:\n";
...
bool CBlock::AcceptBlock()
{
    ...
    if (hashBestChain == hash)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodes)
            if (nBestHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 200 : nBlockEstimate))
...
{
    int64_t ret = blockValue * 8/10; //80%
