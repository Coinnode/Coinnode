...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Coinnode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Coinnoded.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCoinnodeamount=<n> " + _("Keep N Coinnode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Coinnode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Coinnode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Coinnode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Coinnode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Coinnode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Coinnode to complete") << "\n";
    ...
    nAnonymizeCoinnodeAmount = GetArg("-anonymizeCoinnodeamount", 0);
    if(nAnonymizeCoinnodeAmount > 999999) nAnonymizeCoinnodeAmount = 999999;
    if(nAnonymizeCoinnodeAmount < 2) nAnonymizeCoinnodeAmount = 2;
    ...
    LogPrintf("Anonymize Coinnode Amount %d\n", nAnonymizeCoinnodeAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CNE+1000 == (.1CNE+100)*10
       10CNE+10000 == (1CNE+1000)*10
    */
