...
//
// Bootup the masternode, look for a 500 CNE input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Coinnode.conf */
...
{
    CCoinnodeAddress address(collateralAddress);
