...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCoinnodeAmount"))
        settings.setValue("nAnonymizeCoinnodeAmount", 1000);
    nAnonymizeCoinnodeAmount = settings.value("nAnonymizeCoinnodeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCoinnodeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCoinnodeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCoinnodeAmount:
            return QVariant(nAnonymizeCoinnodeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCoinnodeAmount:
            nAnonymizeCoinnodeAmount = value.toInt();
            settings.setValue("nAnonymizeCoinnodeAmount", nAnonymizeCoinnodeAmount);
            emit AnonymizeCoinnodeAmountChanged(nAnonymizeCoinnodeAmount);
            break;
        default:
            break;
        }
