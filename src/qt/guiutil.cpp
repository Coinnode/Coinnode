...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Coinnode"))
        return false;
...
{
    // Convert Coinnode:// to Coinnode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Coinnode://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Coinnode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Coinnode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Coinnode.lnk
...
{
    return GetAutostartDir() / "Coinnode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Coinnode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Coinnode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Coinnode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Coinnode app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Coinnode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Coinnode-qt [" + tr("command-line options") + "]                     " + "\n";
