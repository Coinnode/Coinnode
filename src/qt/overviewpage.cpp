...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCoinnodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCoinnodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCoinnodeAmount = strAnonymizeCoinnodeAmount.remove(strAnonymizeCoinnodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCoinnodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCoinnodeAmount*COIN) nMaxToAnonymize = nAnonymizeCoinnodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCoinnodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCoinnodeAmount));
        strAnonymizeCoinnodeAmount = strAnonymizeCoinnodeAmount.remove(strAnonymizeCoinnodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCoinnodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
